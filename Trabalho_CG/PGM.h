...
public:
	PGM(int l, int a) {
		this->larg = l;
		this->alt = a;
		this->pixels = new unsigned char[this->larg * this->alt];
...
static bool verificarTipoP2(string caminho){
		ifstream arq;
		string aux;
		arq.open(caminho);
			if(!arq.is_open()){
	...
	static void map(PGM* entrada, PGM* saida, Regiao rEntrada, Regiao rSaida) {
	...
	static void mapInv(PGM* entrada, PGM* saida, Regiao rEntrada, Regiao rSaida) {
